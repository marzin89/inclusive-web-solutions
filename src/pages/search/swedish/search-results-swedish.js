...
function SearchResultsSwedish() {
    const activePage = useSelector((state) => state.page.activeSearchPageSwedish);
    const page = useSelector((state) => state.page.pageSwedish);
                    ...
                    <article key={result.id}>
                        ...
                        <p><Link id={result.foreignKey ? result.foreignKey : null} className="find-out-more 
                            regular-font-size focus focus-invisible" to={`${result.path}`} 
                                onClick={(e) => handleLinkClick(e)}>Läs mer</Link></p>
                    </article>
                );
            })}
        </div>
...
export default SearchResultsSwedish;