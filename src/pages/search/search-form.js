...
function SearchForm(props) {
    ...
    const numberOfResults = useSelector((state) => props.language == 'Swedish' ?
        state.page.searchResultsSwedish.length : state.page.searchResultsGerman.length);
    const [errorMessage, setErrorMessage] = useState();
    const dispatch        = useDispatch();

    const labelText  = props.language == 'Swedish' ? 'Sök på webbplatsen' : 'Website durchsuchen';
    const buttonText = props.language == 'Swedish' ? 'Sök' : 'Suchen';
    const resultText = props.language == 'Swedish' ? ' träffar' : ' Treffer';
    ...
    function search() {
        ...
        */
        ...
        .catch(() => {
            dispatch(pageActions.setErrorMessage(props.language == 'Swedish' ? 
                'Ett serverfel har uppstått. Din sökning kunde inte genomföras. Försök igen lite senare.'
                : 'Ein Serverfehler ist aufgetreten. Ihre Suche konnte leider nicht durchgeführt werden.'
    ...
    return (
        <form role="search" onSubmit={(e) => handleSubmit(e)}>
            <label htmlFor="search-bar-main">{labelText}</label>
            ...
            <button className="search-btn search-btn-mobile btn focus focus-invisible-input regular-font-size" 
                type="submit">{buttonText}</button>
            {numberOfResults ? <p id="number-of-results" className="number-of-results 
                regular-font-size" role="alert">{numberOfResults + resultText}</p> : null}
