...
function SearchResults(props) {
    ...
    const page = useSelector((state) => props.language == 'Swedish' ?
        state.page.pageSwedish : state.page.pageGerman);
                ...
                return (
                    <article key={result.id}>
                        ...
                        <p className="regular-font-size line-height">{result.content.slice(0, 150) 
                            + ' ...'}</p>
                        <p><Link id={result.foreignKey ? result.foreignKey : null} className="find-out-more regular-font-size 
                            focus focus-invisible" to={`${result.path}`} onClick={(e) => handleLinkClick(e)}>
